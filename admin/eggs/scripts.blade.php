...
@section('content-header')
    <h1>{{ $egg->name }}<small>Управляйте скриптом установки для этого Яйца.</small></h1>
    <ol class="breadcrumb">
                ...
                @if(! is_null($egg->copyFrom))
                    <div class="box-body">
                        <div class="callout callout-warning no-margin">
                            Эта опция сервиса копирует скрипты установки и опции контейнера из <a href="{{ route('admin.nests.egg.view', $egg->copyFrom->id) }}">{{ $egg->copyFrom->name }}</a>. Любые изменения, которые вы внесете в этот скрипт, не будут применяться, если вы не выберете "Нет" в выпадающем списке ниже.
                        </div>
                    </div>
                @endif
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="form-group col-sm-4">
                            ...
                            <p class="text-muted small">Если выбрано, скрипт выше будет проигнорирован, и вместо него будет использоваться скрипт из выбранного варианта.</p>
                        </div>
                        <div class="form-group col-sm-4">
                            ...
                            <p class="text-muted small">Контейнер Docker для использования при запуске этого скрипта для сервера.</p>
                        </div>
                        <div class="form-group col-sm-4">
                            <label class="control-label">Команда Запуска Скрипта</label>
                            <input type="text" name="script_entry" class="form-control" value="{{ $egg->script_entry }}" />
                            <p class="text-muted small">Команда запуска для этого скрипта.</p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 text-muted">
                            Следующие опции сервиса зависят от этого скрипта:
